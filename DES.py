...
def leftCircularShift(val, counter):
    Left = val[0:28]
    Right = val[28:56]
    Right = (Right[BS[counter]:] + Right[:BS[counter]]).zfill(28)
    Left = (Left[BS[counter]:] + Left[:BS[counter]]).zfill(28)
    return Left + Right
...
def bits2string(b=None):
    return ''.join([chr(int(x, 2)) for x in [b[i*8:i*8+8] for i in range(8)]])


def createSubKeys(key):
    # create 16 subkeys
    # first permutation
    key_PC1 = permutation(key, PC1)
    shift_keys = []
    # shift bits
